...
@interface BeaconBarcodeScnVC ()<URLManagerDelegate>
{
...
@implementation BeaconBarcodeScnVC
...
- (void) readerView:(ZBarReaderView *)readerView didReadSymbols: (ZBarSymbolSet *)symbols fromImage:(UIImage *)image
{
    ...
    for (s in symbols)
    {
        ...
        {
            msg = @"This is not valid BleID Number.";
        }
        else if ([scanedTxt length] <12)
        {
            msg=@"This Beacon number is invalid. It should be 12 digits.";
        }
        else if ([scanedTxt length] >11)
        {
            msg=@"This Beacon number is invalid. It should be 12 digits.";
        }
        else
        {
            txtBleID.text = scanedTxt;
            installScanID = scanedTxt;
            strLocalBleNum = scanedTxt;
            if ([APP_DELEGATE isNetworkreachable])
            {
                //Call Service to get Device details
//                [self getDeviceIDfromBLERomanAPI];
...
-(void)setManualView
{
    ...
    if (isManual)
    {
        lblmessage.text= @"Please enter Beacon  device number  manually";
    }
    else
    {
        lblmessage.text= @"Your device does not support Camera for barcode scan. Please enter device Beacon number manually";
    }
    ...
    txtBleID = [[UITextField alloc] initWithFrame:CGRectMake(20, yy, viewMore.frame.size.width-40, 35)];
    txtBleID.placeholder = @"Enter Beacon number";
    txtBleID.autocapitalizationType = UITextAutocapitalizationTypeNone;
    txtBleID.delegate = self;
    txtBleID.autocorrectionType = UITextAutocorrectionTypeNo;
//    txtBleID.keyboardType = UIKeyboardTypeNumberPad;
    txtBleID.textColor = [UIColor whiteColor];
    [txtBleID setFont:[UIFont fontWithName:CGRegular size:14]];
    [APP_DELEGATE getPlaceholderText:txtBleID andColor:UIColor.whiteColor];
    [viewMore addSubview:txtBleID];
    
    UILabel * lblEmailLine = [[UILabel alloc] initWithFrame:CGRectMake(0, txtBleID.frame.size.height-2, txtBleID.frame.size.width, 1)];
    [lblEmailLine setBackgroundColor:[UIColor lightGrayColor]];
    [txtBleID addSubview:lblEmailLine];
...
{
    [txtBleID resignFirstResponder];
    if ([txtBleID.text length] == 0 || txtBleID.text == nil || [txtBleID.text isEqualToString:@" "])
    {
        lblerror.text=@"Please enter Beacon number";
        lblerror.hidden=NO;
    }
    else if ([txtBleID.text length] <12)
    {
        lblerror.text=@"Please enter 12 digit Beacon number";
        lblerror.hidden=NO;
    }
    else if ([txtBleID.text length] >12)
    {
        lblerror.text=@"Beacon number should not be greater than 12 digits";
        lblerror.hidden=NO;
    }
    else if ([txtBleID.text rangeOfString:@" "].location != NSNotFound)
    {
        lblerror.text=@"Please remove white space from Beacon number";
        lblerror.hidden=NO;
    }
    else if ([txtBleID.text rangeOfString:@"%"].location != NSNotFound)
    {
        lblerror.text=@"Please remove % from Beacon number";
        lblerror.hidden=NO;
    }
    else
    {
        ...
        viewMore.hidden = YES;
        lblerror.hidden=YES;
        installScanID = txtBleID.text;
        strLocalBleNum = txtBleID.text;
        //Call Service to get Device details
...
//            [self getDeviceIDfromBLERomanAPI];
            ...
            [globalBeaconInstVC GetLocalBleNUmber:strLocalBleNum];
            [self.navigationController popViewControllerAnimated:YES];
...
-(void)getDeviceIDfromBLERomanAPI
{
    if ([APP_DELEGATE isNetworkreachable])
    {
    ...
    dispatch_async(queue, ^{
        {
            ...
            AFHTTPRequestOperation *op = [manager1 GET:strUrl parameters:args success:^(AFHTTPRequestOperation *operation, id responseObject){
                ...
                else
                {
                    if ([dictID count]>=10)
                    {
                        NSString * beaconstatus = [self checkforValidString:[dictID valueForKey:@"status"]];
                        if (![beaconstatus isEqualToString:@"NA"])
                        {
                            beaconstatus = [beaconstatus uppercaseString];
                            if ([beaconstatus isEqualToString:@"DELIVERED"] || [beaconstatus isEqualToString:@"UNINSTALL"])
                            {
//                                [globalbeaconVC GetBeaconNumber:dictID];
//                           [globalBeaconInstVC GetBeaconNumber:dictID];

                                //Sucess pop up saying Beacon is ready to install. Nd move back

                                URBAlertView *alertView = [[URBAlertView alloc] initWithTitle:ALERT_TITLE message:@"Beacon is ready to install." cancelButtonTitle:OK_BTN otherButtonTitles: nil, nil];
                                [alertView setMessageFont:[UIFont fontWithName:CGRegular size:14]];
                                [alertView setHandlerBlock:^(NSInteger buttonIndex, URBAlertView *alertView) {
                                    [alertView hideWithCompletionBlock:^{
...
//    [UIView setAnimationTransition:UIViewAnimationTransitionFlipFromLeft forView:[[UIApplication sharedApplication] keyWindow] cache:YES];
                     ...
                     if (IS_IPHONE_X){[alertView showWithAnimation:URBAlertAnimationDefault];}
                            ...
                            else if ([beaconstatus isEqualToString:@"INSTALLED"])
                            {
                                //error : This beacon is already installed somewhere else. You can't install it untill Uninstalled.
                                URBAlertView * alert =[[URBAlertView alloc] initWithTitle:ALERT_TITLE message:@"This beacon is already installed somewhere else. You can't install it untill Uninstalled." cancelButtonTitle:OK_BTN otherButtonTitles:nil, nil];
                                [alert showWithAnimation:URBAlertAnimationTopToBottom];
                            }
                            else
                            {
                                //show Error that "This beacon can't be installed."
                                URBAlertView * alert =[[URBAlertView alloc] initWithTitle:ALERT_TITLE message:@"This beacon can't be installed." cancelButtonTitle:OK_BTN otherButtonTitles:nil, nil];
                                                          [alert showWithAnimation:URBAlertAnimationTopToBottom];
                            }
                        }
                        else
                        {
                            //show alert invalid beacon number
                            URBAlertView * alert =[[URBAlertView alloc] initWithTitle:ALERT_TITLE message:@"Invalid Beacon number." cancelButtonTitle:OK_BTN otherButtonTitles:nil, nil];
                            [alert showWithAnimation:URBAlertAnimationTopToBottom];
                        }
                    }
                    else
...
//                    [dtaliDict setObject:strLocalBleNum forKey:@""];// you have to send the bleNumber
                    [dtaliDict setObject:_date forKey:@"date"];
    ...
    if ([[result valueForKey:@"commandName"] isEqualToString:@"checkdevice"])
    {
        ...
        if (IS_IPHONE_X) {[alertView showWithAnimation:URBAlertAnimationDefault];}
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        NSString * strMessage = [NSString stringWithFormat:@"This device %@ is already installed somewhere. Please uninstall first and then try again to Install it.",strLocalBleNum];
        ...
        if (IS_IPHONE_X) {[alertView showWithAnimation:URBAlertAnimationDefault];}
                    ...
                    else if ([[tmpDict valueForKey:@"status"] isEqualToString:@"2"] || [[tmpDict valueForKey:@"status"] isEqualToString:@"0"])
                    {
                        ...
                        NSString * strMessage = [NSString stringWithFormat:@"This device %@ is ready to use.",strLocalBleNum];
...
//                                    globalWarrantyStatus =[tmpDict valueForKey:@"warranty"];
        ...
        else
        {
            ...
            else if ([[[result valueForKey:@"result"] valueForKey:@"message"] isEqualToString:@"Device not found with this bleNumber!"])
            {
                NSString * strMessage = [NSString stringWithFormat:@"This BLe code doesn't exist with us. Please check IMEI code."];
    ...
    else if ([[result valueForKey:@"commandName"] isEqualToString:@"testdevice"])
    {
        if ([[[result valueForKey:@"result"] valueForKey:@"response"] isEqualToString:@"true"])
        {
            if ([[[result valueForKey:@"result"] valueForKey:@"message"] isEqualToString:@"Device not exists with this IMEI!"])
            {
                ...
                NSString * strMessage = [NSString stringWithFormat:@"This device %@ can't be uninstalled as its not installed anywhere.",strLocalBleNum];
                if ([isFromInstall isEqualToString:@"Inspection"])
                {
                    strMessage = [NSString stringWithFormat:@"This device %@ can't be inspected as its not installed anywhere.",strLocalBleNum];
                }
                URBAlertView *alertView = [[URBAlertView alloc] initWithTitle:ALERT_TITLE message:strMessage cancelButtonTitle:OK_BTN otherButtonTitles:@"View last Installation", nil];
                [alertView setMessageFont:[UIFont fontWithName:CGRegular size:12]];
                [alertView setHandlerBlock:^(NSInteger buttonIndex, URBAlertView *alertView) {
